...
| 評価種別 | 詳細設計書 |
|---------|----------|
| 日本語能力評価（語彙・文法・内容・敬語） | 07_評価ロジック 7.1〜7.4 |
| 採用適性評価（適応力・コミュニケーション力・主体性・定着意向・協調性） | 13_面接シナリオ設計 13.2 |
| フィードバック生成・苦手分析 | 07_評価ロジック 7.5 |
...
> **詳細**: エラーハンドリングの詳細フローは **12_面接フロー制御 12.7** を参照
...
> **詳細**: 話速設定の実装詳細は **12_面接フロー制御 12.2** を参照

---

## 9.4 mintoku work（連携サービス）

> **注意**: 本節のmintoku work連携仕様（APIエンドポイント、認証方式、データ形式等）は、設計時点での想定に基づく。mintoku work側のAPI仕様が正式公開された時点で、本節の内容を確定・更新すること。
...
> **送信データ形式**
> ※ 詳細な送信データ形式（JSON構造）は [10_API仕様 10.6](./10_API仕様.md#106-外部api連携mintoku-work) を参照
...
| 学習計画生成 | 希望業界に基づいて自動生成（詳細は12_面接フロー制御 12.10.3〜12.10.4） |
...
ユーザーのログイン状態を確認
   ...
   │   → 業界変更確認ダイアログ表示（12_面接フロー制御 12.10.8参照）
   │
   └─ 非ログイン（セッションなし）
...
> **詳細**: ログイン時の処理フローは **12_面接フロー制御 12.10.9** を参照
