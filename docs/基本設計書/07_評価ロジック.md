...
| No | 評価種別 | 設計書 | 評価対象 | スコア範囲 | 評価タイミング |
|----|---------|--------|---------|----------|--------------|
| 1 | **日本語能力評価** | 本章 | 語彙・文法・内容・敬語 | 0-100点 | 各回答ごとにGPT-4oで自動評価 |
| 2 | **採用適性評価** | 13章 | 適応力・コミュニケーション力・主体性・定着意向・協調性 | 1-5点 | セッション終了後に総合評価 |
...
| No | レベル | 評価の重点項目 | AIによる主なチェック項目                             |
|----|--------|----------------|---------------------------------------------------- |
| 1 | N1/N2  | ビジネス即戦力 | 尊敬・謙譲語の使い分け、論理的思考、逆質問の妥当性   |
| 2 | N3     | 実務・接客対応 | 丁寧語（です・ます）の維持、質問の意図解釈、定型表現 |
| 3 | N4/N5  | 基本意思疎通   | 基本挨拶、語順の正しさ、聞き取りの可否               |

## 7.2 評価カテゴリ

| No | カテゴリ | 英語名 | 説明 |
|----|---------|--------|------|
| 1 | 語彙 | vocabulary | 適切な語彙の選択、語彙の豊富さ |
| 2 | 文法 | grammar | 文法の正確性、文構造の適切さ |
| 3 | 内容 | content | 回答の論理性、質問への適切な応答 |
| 4 | 敬語 | honorifics | 敬語の正確な使用、場面に応じた使い分け |
...
| No | レベル | 語彙 | 文法 | 内容 | 敬語 |
|----|--------|------|------|------|------|
| 1 | N1 | 0.20 | 0.20 | 0.25 | 0.35 |
| 2 | N2 | 0.20 | 0.25 | 0.25 | 0.30 |
| 3 | N3 | 0.25 | 0.30 | 0.25 | 0.20 |
| 4 | N4 | 0.30 | 0.35 | 0.25 | 0.10 |
| 5 | N5 | 0.35 | 0.40 | 0.20 | 0.05 |
...
3. **蓄積**: 同一カテゴリの苦手が**累計3回以上**発生した場合、苦手タグとしてDBに記録
   - **累計3回**: 連続である必要はない。異なるセッションで合計3回70点未満を記録した場合に苦手判定
...
| 状態 | 条件 | 次の状態 |
|------|------|----------|
| 正常 | スコア70点未満が**累計3回**発生 | 苦手（タグ登録） |
| 苦手 | スコア70点以上が**3回連続** | 解消（resolved = true） |
| 解消済み | スコア70点未満が**累計3回**再発生 | 苦手（タグ再登録） |

> **検出と解消の違い**:
> - **苦手検出**: 累計3回（連続でなくてよい）で判定。苦手傾向を早期に検出するため。
> - **苦手解消**: 3回連続で判定。安定して改善したことを確認するため。
...
両評価を統合したデータをmintoku workに送信する。詳細は10章 10.6を参照。
...
> スコアからの「推定レベル算出」については7.9.5を正式な定義として参照すること。
...
> - **総合推定実力レベル**: 7.11.2で定義。複数セッションの履歴から算出する総合的な推定。
...
> **用語の区別**:
> - **単一セッション推定レベル**: 7.9.5で定義。1回のセッションスコアから算出する簡易推定。
...
> **注意**: 以下のファイルパスは設計時点での想定であり、実装時に変更される可能性がある。

| ファイル | 役割 |
|----------|------|
| `frontend/src/types/interview.ts` | 型定義（EnterpriseEvaluationReport等） |
| `frontend/src/hooks/useAdaptiveLevel.ts` | 推定ロジック・レポート生成 |
| `frontend/src/components/FeedbackDisplay.tsx` | 企業向けレポートUI |
