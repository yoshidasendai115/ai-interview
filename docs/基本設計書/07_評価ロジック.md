...
### 7.7.2 統合スコアの計算方法（正式定義）

> **注意**: 統合スコア計算の唯一の正式定義はこのセクションである。
> 他の設計書（12章、13章）ではこのセクションを参照すること。
...
| 評価スコア | 表示ボタン | アクション |
|-----------|-----------|----------|
| **70点以上** | 「N○にチャレンジ！」 | 自動で1つ上のレベルに移行 |
| **31〜69点** | 「同じレベルで再挑戦」＋「上のレベルにチャレンジ」 | ユーザーが選択 |
| **30点以下** | 「N○で練習する」 | 自動で1つ下のレベルに移行 |

### 7.10.4 チャレンジ枠機能

申告レベルより1つ上のレベルで最初から挑戦できる「チャレンジ枠」を提供。
...
- 面接開始画面に「チャレンジ枠で挑戦（N○）」ボタンを表示
- 残り回数を「残り2/3回」のように表示
- 制限に達した場合は「本日のチャレンジ枠は終了しました」を表示
...
│  基本接客（N4相当）     : ✅ 十分対応可能                      │
│  一般業務（N3相当）     : ✅ 十分対応可能                      │
│  ビジネス敬語（N2相当） : ⚠️ 対応可能（要練習）                │
│  高度業務（N1相当）     : ❓ 未検証                            │
│                                                               │
│ 【4. 詳細スコア】                                             │
│ ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ │
│  語彙: 82点  文法: 78点  内容: 88点  敬語: 65点               │
│  苦手項目: 敬語（謙譲語の使い分け）                           │
└─────────────────────────────────────────────────────────────┘
```

### 7.11.3 推定実力レベルの算出ロジック
